...
export default class MobileconfigApi {
    static async mobileConfigDoH(host: string, client_id?: string): Promise<number | Error> {
        ...
        return await fetch(`/api/v1/apple/doh.mobileconfig?${qs.stringify(queryParams, { arrayFormat: 'comma' })}`, {
            method: 'GET',
        }).then(async (res) => {
            if (res.status === 200) {
                return res.status;
    ...
    static async mobileConfigDoT(host: string, client_id?: string): Promise<number | Error> {
        ...
        return await fetch(`/api/v1/apple/dot.mobileconfig?${qs.stringify(queryParams, { arrayFormat: 'comma' })}`, {
            method: 'GET',
        }).then(async (res) => {
            if (res.status === 200) {
                return res.status;
