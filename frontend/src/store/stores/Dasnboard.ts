...
export default class Dashboard implements IStore {
    ...
    * getStats() {
        const response: IStats | Error = yield statsApi.stats();
    ...
    * getClient(ip: string) {
        ...
        const response: Error | IClientsFindEntry[] = yield clientsApi.clientsFind(ip);
    ...
    * getTlsConfig() {
        const response: ITlsConfig | Error  = yield tlsApi.tlsStatus();
