...
export default class System implements IStore {
    ...
    * getProfile() {
        const response: Error | IProfileInfo = yield globalApi.getProfile();
    ...
    * getServerStatus() {
        const response: IServerStatus |Error = yield globalApi.status();
    ...
    * switchServerStatus(enable: boolean) {
        const response: number | Error | string[] = yield globalApi.dnsConfig({
