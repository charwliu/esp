...
func TestLedgerScannerBasic(t *testing.T) {
	...
	parsed, err := io.ReadAll(r)
	if err != nil {
		t.Fatal(err)
	}

	expected, _ := os.ReadFile(filepath.Join("testdata", "ledgerReader_expected_0"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLedgerScannerSingleInclude(t *testing.T) {
	...
	expected, err := os.ReadFile(filepath.Join("testdata", "ledgerReader_expected_1"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLedgerScannerWildcardInclude(t *testing.T) {
	...
	expected, err := ioutil.ReadFile(filepath.Join("testdata", "ledgerReader_expected_wildcard"))
	if err != nil {
		t.Fatal(err)
	}
