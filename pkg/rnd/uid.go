...
// IsLowerAlphanumeric returns true if the string only contains alphanumeric ascii chars without whitespace.
...
// IsPPID returns true if string is a unique id as generated by Gosh.
func IsPPID(s string, prefix byte) bool {
	if len(s) != 16 {
		return false
	}

	if !IsLowerAlphanumeric(s) {
		return false
	}
